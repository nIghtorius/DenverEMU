...
void	denvergui::internal_render_dbg_node(dbg_dt* node) {
	...
	const int32_t* i32data = (const int32_t*)node->data;
	const uint32_t* ui32data = (const uint32_t*)node->data;
	...
	case t_int32:
		ImGui::Text("%s:", node->description.c_str()); ImGui::SameLine();
		ImGui::Text("%d", *i32data);
		break;
	case t_uint32:
		ImGui::Text("%s:", node->description.c_str()); ImGui::SameLine();
		ImGui::Text("%d", *ui32data);
		break;
	case t_byte:
		if (node->max == SHOW_ONLY_VALUE_AS_TEXT) {
			ImGui::Text("%s:", node->description.c_str()); ImGui::SameLine();
...
void	denvergui::render_cpuviewer(nes_emulator *denver, denvergui_state *state) {
	if (ImGui::Begin("CPU debugger", &state->show_cpu_debugger)) {
		...
		if (ImGui::TreeNode("Disassembler")) {
			...
			if (ImGui::Checkbox("Lock on CPU,r.PC", &state->lock_disasm_cpu)) {
				//state->lock_disasm_cpu = !state->lock_disasm_cpu;
			}
			if (state->lock_disasm_cpu) state->disasm_start = denver->nes_2a03->cpu_2a03.regs.pc;
