...
void mmc1_rom::update_banks() {
	...
	case MMC1_PRG_FIRSTBANK_FIX_8000:
		bank = state.prgbank;
		prg8000 = &romdata[0];
		prgC000 = &romdata[(bank << 14)%(romsize-16384)];
		break;
	case MMC1_PRG_LASTBANK_FIX_C000:
		bank = state.prgbank;
		prg8000 = &romdata[(bank << 14)%(romsize-16384)];
...
void mmc1_vrom::update_banks(mmc1_state &state) {
	...
	case MMC1_CHR_4K_MODE:
		bank = state.chrbank0;
		chr0000 = &romdata[(bank << 12)%(romsize-4096)]; // 4kB blocks.
		bank = state.chrbank1;
		chr1000 = &romdata[(bank << 12)%(romsize-4096)]; // 4kB blocks.
		break;
	}
}
