...
void cpu2a03_fast::pushstack_byte(byte data) {
	...
	mbus->writememory(0x0100 + regs.sp, data);
	regs.sp--;
}
...
byte cpu2a03_fast::pullstack_byte() {
	regs.sp++;
	return mbus->readmemory(0x0100 + regs.sp);
}
...
int	cpu2a03_fast::rundevice_internal (int ticks) {
	...
	if (mbus->nmi_pulled()) {
		...
		regs.pc = mbus->readmemory_as_word(vector_nmi); // NMI vector.
		return 7 * tick_rate;	// NMI takes 7 cycles.
	}
	// IRQ
	if ((mbus->irq_pulled()) && ((regs.sr & cf_interrupt) == 0)) {
		pushstack_word(regs.pc);
		pushstack_byte(regs.sr & ~0x10);
		regs.sr |= cf_interrupt;
		regs.pc = mbus->readmemory_as_word(vector_irq); // IRQ vector.
		return 7 * tick_rate;	// IRQ takes 7 cycles.
	}	

	byte opcode = mbus->readmemory(regs.pc);
	regs.pc++;
	switch (opcode) {
		case 0x00: {	// BRK instruction
			...
			regs.sr |= cf_interrupt;
			regs.pc = mbus->readmemory_as_word(vector_irq);	// load brk vector in regs.pc
			break;
		}
		case 0x01: {
			opc_ora(mbus->readmemory(_mem_index_x));
			regs.pc++;
			actualticks += 6;
			break;
		}
		...
		case 0x05: {
			opc_ora(mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		...
		case 0x09: {
			opc_ora(mbus->readmemory(_mem_immediate));
			regs.pc++;
			actualticks += 2;
			break;
		}
		...
		case 0x0b: case 0x2b: {// aac?
			opc_and(mbus->readmemory(_mem_immediate));
			regs.pc++;
		...
		case 0x0d: {
			opc_ora(mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		...
		case 0x11: {
			opc_ora(mbus->readmemory(_mem_index_y));
			regs.pc++;
			actualticks += 5;
		...
		case 0x15: {
			opc_ora(mbus->readmemory(_mem_zero_x));
			regs.pc++;
			actualticks += 4;
			break;
		}
		...
		case 0x19: {
			opc_ora(mbus->readmemory(_mem_absolute_y));
			regs.pc += 2;
			actualticks += 4;
		...
		case 0x1d: {
			opc_ora(mbus->readmemory(_mem_absolute_x));
			regs.pc += 2;
			actualticks += 4;
		...
		case 0x21: {
			opc_and(mbus->readmemory(_mem_index_x));
			regs.pc++;
			actualticks += 6;
			break;
		}
		...
		case 0x24: {
			opc_bit(mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0x25: {
			opc_and(mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		...
		case 0x2c: {
			opc_bit(mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		case 0x2d: {
			opc_and(mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		...
		case 0x31: {
			opc_and(mbus->readmemory(_mem_index_y));
			regs.pc++;
			actualticks += 5;
			if (_mem_index_y_inc) actualticks++;
			break;
		...
		case 0x35: {
			opc_and(mbus->readmemory(_mem_zero_x));
			regs.pc++;
			actualticks += 4;
			break;
		}
		...
		case 0x39: {
			opc_and(mbus->readmemory(_mem_absolute_y));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_y & 0xFF00)) actualticks++;
			break;
		...
		case 0x3d: {
			opc_and(mbus->readmemory(_mem_absolute_x));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_x & 0xFF00)) actualticks++;
			break;
		...
		case 0x41: {
			opc_xor(mbus->readmemory(_mem_index_x));
			regs.pc++;
			actualticks += 6;
			break;
		}
		...
		case 0x45: {
			opc_xor(mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		...
		case 0x49: {
			opc_xor(mbus->readmemory(_mem_immediate));
			regs.pc++;
			actualticks += 2;
			break;
		}
		...
		case 0x4d: {
			opc_xor(mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		...
		case 0x51: {
			opc_xor(mbus->readmemory(_mem_index_y));
			regs.pc++;
			actualticks += 5;
			if (_mem_index_y_inc) actualticks++;
			break;
		...
		case 0x55: {
			opc_xor(mbus->readmemory(_mem_zero_x));
			regs.pc++;
			actualticks += 4;
			break;
		}
		...
		case 0x59: {
			opc_xor(mbus->readmemory(_mem_absolute_y));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_y & 0xFF00)) actualticks++;
			break;
		...
		case 0x5d: {
			opc_xor(mbus->readmemory(_mem_absolute_x));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_x & 0xFF00)) actualticks++;
			break;
		...
		case 0x61: {
			opc_adc(mbus->readmemory(_mem_index_x));
			regs.pc++;
			actualticks += 6;
			break;
		}
		...
		case 0x65: {
			opc_adc(mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		...
		case 0x69: {
			opc_adc(mbus->readmemory(_mem_immediate));
			regs.pc++;
			actualticks += 2;
			break;
		}
		...
		case 0x6b: { //arr  (have no idea how it actually works. but ac & imm, ror (ac) general)
			byte data = mbus->readmemory(_mem_immediate);
			data &= regs.ac;
		...
		case 0x6d: {
			opc_adc(mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		...
		case 0x71: {
			opc_adc(mbus->readmemory(_mem_index_y));
			regs.pc++;
			actualticks += 5;
			if (_mem_index_y_inc) actualticks++;
			break;
		...
		case 0x75: {
			opc_adc(mbus->readmemory(_mem_zero_x));
			regs.pc++;
			actualticks += 4;
			break;
		}
		...
		case 0x79: {
			opc_adc(mbus->readmemory(_mem_absolute_y));
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_y & 0xFF00)) actualticks++;
			regs.pc += 2;
			break;
		...
		case 0x7d: {
			opc_adc(mbus->readmemory(_mem_absolute_x));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_x & 0xFF00)) actualticks++;
			break;
		...
		case 0x81: {
			mbus->writememory(_mem_index_x, regs.ac);
			regs.pc++;
			actualticks += 6;
			break;
		}
		case 0x83: {
			mbus->writememory(_mem_index_x, regs.ac & regs.x);
			regs.pc++;
			actualticks += 6;
			break;
		}
		case 0x84: {
			mbus->writememory(_mem_zero, regs.y);
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0x85: {
			mbus->writememory(_mem_zero, regs.ac);
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0x86: {
			mbus->writememory(_mem_zero, regs.x);
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0x87: {
			mbus->writememory(_mem_zero, regs.ac & regs.x);
			regs.pc++;
			actualticks += 3;
			break;
		}
		...
		case 0x8c: {
			mbus->writememory(_mem_absolute, regs.y);
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		case 0x8d: {
			mbus->writememory(_mem_absolute, regs.ac);
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		case 0x8e: {
			mbus->writememory(_mem_absolute, regs.x);
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		case 0x8f: {
			mbus->writememory(_mem_absolute, regs.ac & regs.x);
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		...
		case 0x91: {
			mbus->writememory(_mem_index_y, regs.ac);
			regs.pc++;
			actualticks += 6;
			break;
		}
		...
		case 0x94: {
			mbus->writememory(_mem_zero_x, regs.y);
			regs.pc++;
			actualticks += 4;
			break;
		}
		case 0x95: {
			mbus->writememory(_mem_zero_x, regs.ac);
			regs.pc++;
			actualticks += 4;
			break;
		}
		case 0x96: {
			mbus->writememory(_mem_zero_y, regs.x);
			regs.pc++;
			actualticks += 4;
			break;
		}
		case 0x97: {
			mbus->writememory(_mem_zero_y, regs.ac & regs.x);
			regs.pc++;
			actualticks += 4;
			break;
		}
		...
		case 0x99: {
			mbus->writememory(_mem_absolute_y, regs.ac);
			regs.pc += 2;
			actualticks += 5;
			break;
		...
		case 0x9c: {	// keeps failing 07-abs_xy.nes test, everything documented tells me this is right.
			...
			mbus->writememory(_mem_absolute_x, regs.y & (hb + 1));
			regs.pc += 2;
			actualticks += 5;
			break;
		}
		case 0x9d: {
			mbus->writememory(_mem_absolute_x, regs.ac);
			regs.pc += 2;
			actualticks += 5;
			break;
		}
		case 0x9e: {	// keeps failing 07 - abs_xy.nes test, everything documented tells me this is right.
			byte hb = (_mem_absolute >> 8);
			mbus->writememory(_mem_absolute_y, regs.x & (hb + 1));
			regs.pc += 2;
			actualticks += 5;
			break;
		}
		...
		case 0xa0: {
			opc_ld_(regs.y, mbus->readmemory(_mem_immediate));
			regs.pc++;
			actualticks += 2;
			break;
		}
		case 0xa1: {
			opc_ld_(regs.ac, mbus->readmemory(_mem_index_x));
			regs.pc++;
			actualticks += 6;
			break;
		}
		case 0xa2: {
			opc_ld_(regs.x, mbus->readmemory(_mem_immediate));
			regs.pc++;
			actualticks += 2;
			break;
		}
		...
		case 0xa4: {
			opc_ld_(regs.y, mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0xa5: {
			opc_ld_(regs.ac, mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0xa6: {
			opc_ld_(regs.x, mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		...
		case 0xa9: {
			opc_ld_(regs.ac, mbus->readmemory(_mem_immediate));
			regs.pc++;
			actualticks += 2;
			break;
		}
		...
		case 0xac: {
			opc_ld_(regs.y, mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		case 0xad: {
			opc_ld_(regs.ac, mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		case 0xae: {
			opc_ld_(regs.x, mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		...
		case 0xb1: {
			opc_ld_(regs.ac, mbus->readmemory(_mem_index_y));
			regs.pc++;
			actualticks += 5;
			if (_mem_index_y_inc) actualticks++;
			break;
		...
		case 0xb4: {
			opc_ld_(regs.y, mbus->readmemory(_mem_zero_x));
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0xb5: {
			opc_ld_(regs.ac, mbus->readmemory(_mem_zero_x));
			regs.pc++;
			actualticks += 4;
			break;
		}
		case 0xb6: {
			opc_ld_(regs.x, mbus->readmemory(_mem_zero_y));
			regs.pc++;
			actualticks += 4;
			break;
		}
		case 0xb7: {
			opc_ld_(regs.ac, mbus->readmemory(_mem_zero_y));
			regs.x = regs.ac;
			regs.pc++;
			actualticks += 4;
			break;
		...
		case 0xb9: {
			opc_ld_(regs.ac, mbus->readmemory(_mem_absolute_y));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_y & 0xFF00)) actualticks++;
			break;
		...
		case 0xbc: {
			opc_ld_(regs.y, mbus->readmemory(_mem_absolute_x));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_x & 0xFF00)) actualticks++;
			break;
		...
		case 0xbd: {
			opc_ld_(regs.ac, mbus->readmemory(_mem_absolute_x));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_x & 0xFF00)) actualticks++;
			break;
		...
		case 0xbe: {
			opc_ld_(regs.x, mbus->readmemory(_mem_absolute_y));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_y & 0xFF00)) actualticks++;
			break;
		...
		case 0xc0: {
			opc_cmp(regs.y, mbus->readmemory(_mem_immediate));
			regs.pc++;
			actualticks += 2;
			break;
		}
		case 0xc1: {
			opc_cmp(regs.ac, mbus->readmemory(_mem_index_x));
			actualticks += 6;
			regs.pc++;
			break;
		}
		case 0xc3: {
			byte t = opc_dec(mbus->readmemory(_mem_index_x));
			mbus->writememory(_mem_index_x, t);
			opc_cmp(regs.ac, t);
			regs.pc++;
			actualticks += 8;
			break;
		...
		case 0xc4: {
			opc_cmp(regs.y, mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0xc5: {
			opc_cmp(regs.ac, mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0xc6: {
			mbus->writememory(_mem_zero, opc_dec(mbus->readmemory(_mem_zero)));
			regs.pc++;
			actualticks += 5;
			break;
		}
		case 0xc7: {
			byte t = opc_dec(mbus->readmemory(_mem_zero));
			mbus->writememory(_mem_zero, t);
			opc_cmp(regs.ac, t);
			regs.pc++;
			actualticks += 5;
			break;
		...
		case 0xc9: {
			opc_cmp(regs.ac, mbus->readmemory(_mem_immediate));
			regs.pc++;
			actualticks += 2;
			break;
		}
		...
		case 0xcc: {
			opc_cmp(regs.y, mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		case 0xcd: {
			opc_cmp(regs.ac, mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		case 0xce: {
			mbus->writememory(_mem_absolute, opc_dec(mbus->readmemory(_mem_absolute)));
			regs.pc += 2;
			actualticks += 6;
			break;
		}
		case 0xcf: {
			byte t = opc_dec(mbus->readmemory(_mem_absolute));
			mbus->writememory(_mem_absolute, t);
			opc_cmp(regs.ac, t);
			regs.pc += 2;
			actualticks += 6;
			break;
		...
		case 0xd1: {
			opc_cmp(regs.ac, mbus->readmemory(_mem_index_y));
			regs.pc++;
			actualticks += 5;
			if (_mem_index_y_inc) actualticks++;
			break;
		...
		case 0xd3: {
			byte t = opc_dec(mbus->readmemory(_mem_index_y));
			mbus->writememory(_mem_index_y, t);
			opc_cmp(regs.ac, t);
			regs.pc++;
			actualticks += 8;
			break;
		...
		case 0xd5: {
			opc_cmp(regs.ac, mbus->readmemory(_mem_zero_x));
			regs.pc++;
			actualticks += 4;
			break;
		}
		case 0xd6: {
			mbus->writememory(_mem_zero_x, opc_dec(mbus->readmemory(_mem_zero_x)));
			regs.pc++;
			actualticks += 6;
			break;
		}
		case 0xd7: {
			byte t = opc_dec(mbus->readmemory(_mem_zero_x));
			mbus->writememory(_mem_zero_x, t);
			opc_cmp(regs.ac, t);
			regs.pc++;
			actualticks += 6;
			break;
		...
		case 0xd9: {
			opc_cmp(regs.ac, mbus->readmemory(_mem_absolute_y));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_y & 0xFF00)) actualticks++;
			break;
		...
		case 0xdb: {
			byte t = opc_dec(mbus->readmemory(_mem_absolute_y));
			mbus->writememory(_mem_absolute_y, t);
			opc_cmp(regs.ac, t);
			regs.pc += 2;
			actualticks += 7;
			break;
		...
		case 0xdd: {
			opc_cmp(regs.ac, mbus->readmemory(_mem_absolute_x));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_x & 0xFF00)) actualticks++;
			break;
		...
		case 0xde: {
			mbus->writememory(_mem_absolute_x, opc_dec(mbus->readmemory(_mem_absolute_x)));
			regs.pc += 2;
			actualticks += 7;
			break;
		}
		case 0xdf: {
			byte t = opc_dec(mbus->readmemory(_mem_absolute_x));
			mbus->writememory(_mem_absolute_x, t);
			opc_cmp(regs.ac, t);
			regs.pc += 2;
			actualticks += 7;
			break;
		...
		case 0xe0: {
			opc_cmp(regs.x, mbus->readmemory(_mem_immediate));
			regs.pc++;
			actualticks += 2;
			break;
		}
		case 0xe1: {
			opc_sbc(mbus->readmemory(_mem_index_x));
			regs.pc++;
			actualticks += 6;
			break;
		}
		case 0xe3: {
			byte t = opc_inc(mbus->readmemory(_mem_index_x));
			mbus->writememory(_mem_index_x, t);
			opc_sbc(t);
			regs.pc++;
			actualticks += 8;
		...
		case 0xe4: {
			opc_cmp(regs.x, mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0xe5: {
			opc_sbc(mbus->readmemory(_mem_zero));
			regs.pc++;
			actualticks += 3;
			break;
		}
		case 0xe6: {
			mbus->writememory(_mem_zero, opc_inc(mbus->readmemory(_mem_zero)));
			regs.pc++;
			actualticks += 5;
			break;
		}
		case 0xe7: {
			byte t = opc_inc(mbus->readmemory(_mem_zero));
			mbus->writememory(_mem_zero, t);
			opc_sbc(t);
			regs.pc++;
			actualticks += 5;
		...
		case 0xe9: case 0xeb: {
			opc_sbc(mbus->readmemory(_mem_immediate));
			regs.pc++;
			actualticks += 2;
			break;
		}
		...
		case 0xec: {
			opc_cmp(regs.x, mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		case 0xed: {
			opc_sbc(mbus->readmemory(_mem_absolute));
			regs.pc += 2;
			actualticks += 4;
			break;
		}
		case 0xee: {
			mbus->writememory(_mem_absolute, opc_inc(mbus->readmemory(_mem_absolute)));
			regs.pc += 2;
			actualticks += 6;
			break;
		}
		case 0xef: {
			byte t = opc_inc(mbus->readmemory(_mem_absolute));
			mbus->writememory(_mem_absolute, t);
			opc_sbc(t);
			regs.pc += 2;
			actualticks += 6;
		...
		case 0xf1: {
			opc_sbc(mbus->readmemory(_mem_index_y));
			if (_mem_index_y_inc) actualticks++;
			regs.pc++;
			actualticks += 5;
			break;
		...
		case 0xf3: {
			byte t = opc_inc(mbus->readmemory(_mem_index_y));
			mbus->writememory(_mem_index_y, t);
			opc_sbc(t);
			regs.pc++;
			actualticks += 8;
		...
		case 0xf5: {
			opc_sbc(mbus->readmemory(_mem_zero_x));
			regs.pc++;
			actualticks += 4;
			break;
		}
		case 0xf6: {
			mbus->writememory(_mem_zero_x, opc_inc(mbus->readmemory(_mem_zero_x)));
			regs.pc++;
			actualticks += 6;
			break;
		}
		case 0xf7: {
			byte t = opc_inc(mbus->readmemory(_mem_zero_x));
			mbus->writememory(_mem_zero_x, t);
			opc_sbc(t);
			regs.pc++;
			actualticks += 6;
		...
		case 0xf9: {
			opc_sbc(mbus->readmemory(_mem_absolute_y));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_y & 0xFF00)) actualticks++;
			break;
		...
		case 0xfb: {
			byte t = opc_inc(mbus->readmemory(_mem_absolute_y));
			mbus->writememory(_mem_absolute_y, t);
			opc_sbc(t);
			regs.pc += 2;
			actualticks += 7;
		...
		case 0xfd: {
			opc_sbc(mbus->readmemory(_mem_absolute_x));
			regs.pc += 2;
			actualticks += 4;
			if ((_mem_absolute & 0xFF00) != (_mem_absolute_x & 0xFF00)) actualticks++;
			break;
		...
		case 0xfe: {
			mbus->writememory(_mem_absolute_x, opc_inc(mbus->readmemory(_mem_absolute_x)));
			regs.pc += 2;
			actualticks += 7;
			break;
		}
		case 0xff: {
			byte t = opc_inc(mbus->readmemory(_mem_absolute_x));
			mbus->writememory(_mem_absolute_x, t);
			opc_sbc(t);
			regs.pc += 2;
			actualticks += 7;
...
void cpu2a03_fast::opc_asl(word addr, bool memory) {
	byte data = memory ? mbus->readmemory(addr) : regs.ac;
	regs.sr = data & 0x80 ? regs.sr | cf_carry : regs.sr & (0xFF - cf_carry);
	data = data << 1;
	regs.sr &= 0xFF - (cf_negative | cf_zero);
	regs.sr = data & 0x80 ? regs.sr | cf_negative : regs.sr;
	regs.sr = !data ? regs.sr | cf_zero : regs.sr;
	if (memory) mbus->writememory(addr, data); else regs.ac = data;
}
...
void cpu2a03_fast::reset() {
	regs.pc = mbus->readmemory_as_word(vector_reset);		// load reset vector @ 0xFFFC
	regs.sp = 0xFD;
	regs.sr |= cf_interrupt;
	error_state = false;
}
